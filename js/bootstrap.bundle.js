(function (global, factory) {
  ...
  class Carousel extends BaseComponent {
    ...
    _slide(order, element = null) {
      ...
      if (!activeElement || !nextElement) {
        //        Something unusual is happening, so we stop (exit or handle it differently).
        // To Do: Update the tests that use empty <div> elements so we don’t need to perform this check.
        return;
      }
  ...
  const Default$1 = {
    offset: null,
    // TODO: keep it for backwards compatibility reasons
  ...
  const CLASS_NAME_HIDE = "hide"; // kept here only for backwards compatibility
